...
void demandeDeLaffichageDeHistoriqueTrie ()
{
    ...
    int compteur; // on initialise un compteur pour voir le nombre de fois qu'on exécute un écran
    ...
    if (choixAfficherHistoriqueTrieOuNon == 1)//si la réponse de l'utilisateur est 1 (oui)
    {
        ...
        if (fichierHistorique != NULL)//si le fichier historique
        {
            ...
            while(fgets(lecture, 50, fichierHistorique) != NULL) // on va afficher toutes les lignes du texte jusqu'à la fin
            {
                printf("%s", lecture);
                compteur++;
            }
